...
7. add a new role for the server instance that has the correct settings for s3 and db
8. spin up a new ec2 instance in aws
9. allocate a elastic ip for it
10. add a new entry to your .ssh/config for the host or ip address that you'll use to connect to this server
11. use littlechef to provision the new role to it
12. setup a dns entry for this particular machine to make testing easier
13. add a capistrano entry for this instance so its code and db can be updated
