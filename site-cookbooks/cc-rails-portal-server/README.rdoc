...
= Here is a way to copy a database into AWS

1. add the database host to the security group of the RDS instance
2. run this on the database host: 

mysqldump -u sparks_stag_user -pxxxx --lock-tables=false --add-drop-table --quick --extended-insert staging_sparks_prod | pv | mysql -C -h sparks-staging.cheea3ib6y8u.us-east-1.rds.amazonaws.com -u xxxx -pxxx portal

3. If your database host doesn't have pv then can just remove it. It gives a progress view of the data going through the pipe. You can also watch the database storage size on AWS to watch progress.
